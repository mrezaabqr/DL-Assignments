...
def gradient_check(x, theta, epsilon=1e-7, print_msg=False):
    ...
    # difference =                                # Step 3'
    # YOUR CODE STARTS HERE
    numerator = np.sum(np.square(grad - gradapprox))
    denominator = np.sum(np.square(grad)) + np.sum(np.square(gradapprox))
...
```
    ...
    [92mYour backward propagation works perfectly fine! difference = 1.7045043998949008e-19[0m
