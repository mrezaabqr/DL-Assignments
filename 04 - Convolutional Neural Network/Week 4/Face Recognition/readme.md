...
#     print(negative.shape)
    ...
    pos_dist = tf.reduce_sum(tf.square(tf.subtract(anchor, positive)), axis=-1) # mx128
    
    # Step 2: Compute the (encoding) distance between the anchor and the negative
    neg_dist = tf.reduce_sum(tf.square(tf.subtract(anchor, negative)), axis=-1) # mx128
    
    # Step 3: subtract the two previous distances and add alpha.
    basic_loss = tf.add(tf.subtract(pos_dist, neg_dist),  alpha) # mx1
...
def img_to_encoding(image_path, model):
    ...
    return embedding / np.linalg.norm(embedding, ord=2) # normalizing embedding vector
```
